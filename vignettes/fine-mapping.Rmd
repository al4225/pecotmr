...
|---|---|---|
| Individual-level genotype + phenotype (QTL studies) | `univariateAnalysisPipeline()` | `eqtl_region_example` |
| QTL summary statistics + LD reference | `rssAnalysisPipeline()` | placeholder (real QTL nominal output) |
| GWAS summary statistics + LD reference | `rssAnalysisPipeline()` | placeholder (real GWAS sumstats) |
...
to fit via the `methods` parameter on `univariateAnalysisPipeline()`
(individual-level) or `rssAnalysisPipeline()` / `susieRssPipeline()`
(summary statistics):
...
`postprocessFinemappingFits()` produces a single table with the
...
`rssAnalysisPipeline()`. It bundles:

1. `loadRssData()` — tabix-region-extracts the sumstats and
   standardises column names via a YAML mapping (see the
   [RSS QC vignette](rss-qc.html) for the YAML schema).
2. `summaryStatsQc()` — runs allele harmonization, SLALOM (default)
   or DENTIST QC, and optional RAISS imputation. The vignette referenced
   above covers QC in detail.
3. `susieRssPipeline()` — fits the two-stage SuSiE-RSS model and
...
loading and use `susieRssPipeline()` directly with the sumstats and
LD already in memory. This is what `rssAnalysisPipeline()` calls
internally after QC.
...
GWAS fine-mapping uses the **same** `rssAnalysisPipeline()` /
`susieRssPipeline()` call. The only differences are at the loading
layer:
...
produced by `rssAnalysisPipeline()` instead.
...
- **Enrichment** (`xqtlEnrichmentWrapper`): computes prior enrichment
