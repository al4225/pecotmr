...
test_that("sdpr errors on invalid per_variant_sample_size", {
  expect_error(
    sdpr(bhat = rnorm(5), LD = list(blk1 = diag(5)), n = 100,
         perVariantSampleSize = c(100, -1, 100, 100, 100)),
    "positive values"
  )
})
...
test_that("sdpr with per_variant_sample_size", {
  ...
  R <- diag(p)
  result <- sdpr(bhat = bhat, LD = list(blk1 = R), n = 100,
                 perVariantSampleSize = rep(100, p),
                 iter = 50, burn = 10, thin = 2, verbose = FALSE)
  expect_equal(length(result$betaEst), p)
  expect_true(all(is.finite(result$betaEst)))
})
...
test_that("sdpr recovers signal direction on simulated genotype data", {
  ...
  expect_gt(cor(result$betaEst, beta_true), 0.3)
})
...
# ---- sdpr optLlk = 2 (multi-array) ----
test_that("sdpr runs with optLlk = 2 and mixed array values", {
  set.seed(42)
  p <- 10
  bhat <- rnorm(p, sd = 0.1)
  R <- diag(p)
...
test_that("sdpr optLlk = 2 with realistic genotype data", {
  set.seed(2024)
  n <- 500
  p <- 20
  X <- matrix(rbinom(n * p, 2, 0.3), nrow = n)
  ...
  result <- sdpr(bhat = bhat, LD = list(blk1 = R), n = n,
                 perVariantSampleSize = per_n, array = arr, optLlk = 2,
...
# ---- sdprWeights (wrapper) ----
test_that("sdprWeights calls sdpr and returns beta_est", {
  set.seed(42)
  p <- 10
  bhat <- rnorm(p, sd = 0.1)
  R <- diag(p)
  stat <- list(b = bhat, n = rep(100, p))
  result <- sdprWeights(stat = stat, LD = R,
                         iter = 50, burn = 10, thin = 2, verbose = FALSE)
