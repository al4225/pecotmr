...
test_that("format_finemapping_output adds top_loci_export without changing wrapper tables", {
  variant_id <- "chr1:101:A:G"
  result_trimmed <- list(
    pip = setNames(0.8, variant_id),
    alpha = matrix(1, nrow = 1, ncol = 1,
                   dimnames = list("L1", variant_id)),
    mu = matrix(0.8, nrow = 1, ncol = 1,
                dimnames = list("L1", variant_id)),
    mu2 = matrix(0.81, nrow = 1, ncol = 1,
                 dimnames = list("L1", variant_id)),
    V = 1,
  ...
  expect_true("top_loci_export" %in% names(out))
  expect_equal(out$top_loci$betahat, 0.2)
  expect_equal(out$top_loci_export$conditional_effect, 0.8)
  expect_equal(out$top_loci_export$conditional_effect_se, sqrt(0.17),
               tolerance = 1e-8)
  expect_equal(out$top_loci_export$beta, 0.2)
  expect_equal(out$top_loci_export$se, 0.1)
})

test_that("build_top_loci_export expands overlapping CS membership and carries n and af", {
  variant_id <- "chr1:101:A:G"
  result_trimmed <- list(
    pip = setNames(0.8, variant_id),
    alpha = matrix(1, nrow = 1, ncol = 1,
  ...
  af <- setNames(0.35, variant_id)

  out <- pecotmr:::build_top_loci_export(
    top_loci = top_loci,
  ...
  expect_equal(out[["cs_coverage_0.95"]], c(1L, 2L))
  expect_equal(out$gene_ID, c("GENE1", "GENE1"))
  expect_equal(out$event_ID, c("EVENT1", "EVENT1"))
  expect_equal(out$n, c(42, 42))
  expect_equal(out$af, c(0.35, 0.35))
})
