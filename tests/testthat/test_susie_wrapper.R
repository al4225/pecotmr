...
test_that("susieRssPipeline uses beta/se when z not provided", {
  skip_if_not_installed("susieR")
  set.seed(42)
  n <- 15
  beta <- rnorm(n, sd = 0.1)
...
test_that("susieRssPipeline X-mode passes X to susieRss and computes LD from X for post-processor", {
  ...
  local_mocked_bindings(
    ...
    postprocessFinemappingFits = function(fits, dataX, ...) {
      captured_pp_data_x <<- dataX
      list()
    },
    formatFinemappingOutput = function(post, primaryMethod) list()
  )
...
# =============================================================================
# susieRssPipeline: mixture-panel (list of X) branch (Tier 2)
# =============================================================================

test_that("susieRssPipeline computes LD from first panel when X_mat is a list", {
  skip_if_not_installed("susieR")
  p <- 5
  n1 <- 20
  n2 <- 15
  ...
  result <- susieRssPipeline(list(z = z), xMat = X_list)
